...
var CURRENT_URL = window.location.href.split('#')[0].split('?')[0],
  ...
  $LEFT_COL = $('.left_col'),
  $RIGHT_COL = $('.right_col'),
  $SIDE_NAV_MENU = $('#sidebar-menu'),
  
  $CHAT_USERS = $('#chat-users-section'),
  $CHAT_HISTORY = $('#chat-history-section'),
  $CHAT_MSG_SEND = $('#msgsend'),
...
function init_sidebar() {
  ...
  var setContentHeight = function() {
    ...
    $RIGHT_COL.css('min-height', (contentHeight + 2));
    if($LEFT_COL.width() != 90){
    	$SIDE_NAV_MENU.css('max-height', ($LEFT_COL.eq(1).height() - $('.left_col .nav_title').height() - $('.left_col .profile').height()));
    }else{
    	$SIDE_NAV_MENU.css('max-height',"none");
    }
    
    $CHAT_USERS = $('#chat-users-section');
    $CHAT_HISTORY = $('#chat-history-section');
    if($('#chat-users-section').length != 0  && $('#chat-history-section').length != 0){
    	var minHeight = parseInt($SIDE_NAV_MENU.css('max-height'));
    	var msgSendHeight = parseInt($CHAT_MSG_SEND.height()) + 40;
    	$CHAT_USERS.css("max-height",minHeight-20);
    	$CHAT_USERS.css("min-height",minHeight-20);
    	$CHAT_HISTORY.css("max-height",minHeight-msgSendHeight);
    	$CHAT_HISTORY.css("min-height",minHeight-msgSendHeight);
    }
  };
...
function init_daterangepicker() {
  ...
  var optionSet1 = {
    ...
    applyClass: 'btn-lgall btn-primary',
    cancelClass: 'btn-lgall',
    format: 'MM/DD/YYYY',
    separator: ' to ',
    locale: {
      applyLabel: 'Submit',
